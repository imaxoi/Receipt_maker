...
public class PdfReceipt {
    ...
    public PdfReceipt(Receipt receipt) {
        r = receipt;
    ...
    public void preparePdf() throws IOException, DocumentException, IllegalAccessException {
        if (closed)
        ...
        for (int i = 0; i < r.getProducts().size(); i++) {
            Paragraph pDesc = new Paragraph(r.getProducts().get(i).getDescription(), font);
            document.add(pDesc);
            Paragraph pQtyPrice = new Paragraph(Integer.toString(r.getProducts().get(i).getQuantity()) + "x"
                    + Float.toString(r.getProducts().get(i).getPrice()) + "€", font);
        ...
        Paragraph total = new Paragraph("Totale: " + Float.toString(r.getTotal()) + "€", fontTotal);
    ...
    private ArrayList<Paragraph> getHeaders() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Impossibile accedere al file resources/headers.txt");
        }

        String timeStamp = new SimpleDateFormat("dd/MM/yyyy").format(new Date());
        Paragraph dateHeader = new Paragraph("Ricevuta nr. " + r.getID() + " del " + timeStamp, font);
